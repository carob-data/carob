...
# But there is a published paper seem related to the data https://arpgweb.com/pdf-files/jac7(1)7-13.pdf
...
"
	...
	d2 <- r2
	colnames(d2)[colnames(d2) == "Accession_Number"] <- "variety_code"
	colnames(d2)[colnames(d2) == "Accession_Name"]   <- "variety"

	pf <- d2[["Female_AcceNumb"]]
	pm <- d2[["Male_AcceNumb"]]
	...
	syn_list <- lapply(seq_len(nrow(d2)), function(i) {
		nm <- d2$variety[i]
		syns <- if (is.na(nm) || nm == "") character(0) else trimws(strsplit(nm, ";")[[1]])
		unique(toupper(c(d2$variety_code[i], syns)))
	})
	syn_code <- rep(d2$variety_code, lengths(syn_list))
	syn_key  <- unlist(syn_list)
	name_lookup <- setNames(syn_code, syn_key)
	resolved <- name_lookup[toupper(d1$variety_code)]
	unresolved <- is.na(resolved)

	d1$variety_code_original <- d1$variety_code
	d1$variety_code[!unresolved] <- resolved[!unresolved]

	d <- merge(d1, d2, by = "variety_code", all.x = TRUE, sort = FALSE)

	d[c("Numeration", "Family_AcceNumb", "Female_AcceNumb", "Male_AcceNumb",
		"Is_control", "Scale_audpc", "Seed_source", "Simultaneous_trials","Accession_Code",
		"Previous_trials", "list_name", "Date_Created","variety_code_original","Population","Cycle")] <- NULL
