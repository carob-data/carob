...
## ISSUES
#1.Rejected because although the country is available, there is no location/adm1/adm2/adm3
...
"
	...
	f1 <- ff[basename(ff) == "treatment-acai.csv"]
	f2 <- ff[basename(ff) == "column_dictionary_acai_spt_forckan.csv"]
...
## select the variables of interest and assign them to the correct name
	...
	d1 <- data.frame(
		treatment = r1[["factors.treatments..SPT.1.to.SPT.5"]]
	)
##r1: "X"
...
##r2: "Tab", "Column", "description_abstract", "title", "number.of.columns", "Measurement", "data_type", "creator", "source", "source.date", "source.file", "identifier", "subject", "subject.agrovoc", "format", "language", "relation", "coverage", "rights"
