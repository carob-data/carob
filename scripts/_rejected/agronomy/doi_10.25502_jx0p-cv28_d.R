...
## ISSUES
# was placed in the REJECT folder because this is not an experiment and does not contain any relevant agronomic data.


carob_script <- function(path) {

"
Seed yam storage in ambient condition versus 16˚C cold room environment
...
## only keep the comments you added that are specific to this dataset
	...
	f1 <- ff[basename(ff) == "storageanalysiswithnotreatment.csv"]
	f2 <- ff[basename(ff) == "storageanalysiswithnotreatment_data_dictionary.csv"]
...
##r1: "ID", "REP", "Size", "ENVIR", "NOSPT", "Age"


	d2 <- data.frame(
		country = r2[["coverage.country"]]
	)
##r2: "Tab", "Column", "description_abstract", "title", "data_type", "creator", "source", "source.date", "source.file", "identifier", "subject", "subject.agrovoc", "format", "language", "relation", "coverage", "rights"
