 ...
 #general data
	...
	d1$harvest_date <- as.Date(
	  sprintf(
	    ...
	    month1_num,
	    r1$date_harvest_dd_technician_1
	  )
	)
	...
	for(p in 1:8){
	  ...
	  tmp <- data.frame(
	    ...
	    treatment =
	      r2[[paste0("description_treatment_", p)]],
	    
	    variety =
	      r2$experimental_treatments_variety_crop_1,
	    
	    fert_1_kg =
	      r2[[paste0(
	        "fert_1_kg_plot_plot_",
	        p,
	        ".kg_per_plot"
	      )]],
	    ...
	    fert_2_kg =
	      r2[[paste0(
	        "fert_2_kg_plot_plot_",
	        p,
	        ".kg_per_plot"
	      )]],
	    
	    if(p == 1){
	      plot_width <- r2[["width_of_harvested_plot_crop_1_plot_1"]]
	    } else {
	      plot_width <- r2[[paste0(
	        "width_of_harvested_plot_crop_1_plot_",p,".m"
	      )]]
	    },
	    
	    plot_length =
	      r2[[paste0(
	        "depth_of_harvested_plot_perpen_dicular_to_rows_crop_1_plot_",
	        p,
	        ".m"
	      )]],
	    ...
	    dmy_residue =
	      r2[[paste0(
	        "above_ground_biomass_weight_husks_stover_res_crop_1_plot_",
	        p,
	        ".kg"
	      )]],
	...
	d2$fertilizer_type <- apply(
	  r2[c(
	    "experimental_treatments_fertilizer_1",
	    "experimental_treatments_fertilizer_2",
	    "experimental_treatments_fertilizer_3"
	  )],
	  1,
	  function(x){
	...
	d2$OM_type <- rep(
	  r2$experimental_treatments_type_of_manure,
	  each = 8
	)
	...
	for(i in 1:3){
	  ...
	  tmp <- data.frame(
	    ...
	    field_size = ifelse(
	      r3$farm_size_unit == "acre",
	      r3$farm_size * 0.404686,
	      r3$farm_size
	    ),
	    ...
	    yield =
	      r3[[paste0("yield_main_crop_", i)]] /
	      ifelse(
	        r3[[paste0("area_main_crop_", i)]] > 0,
	        r3[[paste0("area_main_crop_", i)]],
	        NA
	      ) *
	      2.47105,
	...
	for(p in 1:4){
	  ...
	  tmp <- data.frame(
	    ...
	    plot_area =
	      r4[[paste0("area_field_", p)]] * 4046.85642,
	    
	    walking_distance =
	      r4[[paste0(
	        "walking_distance_field_",
	        p,
	        ".minutes"
	      )]],
	    ...
	    fertilizer_amount =
	      r4[[paste0(
	        "amount_of_mineral_fertilizer_applied_field_",
	        p
	      )]],
	    
	    fertilizer_type =
	      r4[[paste0(
	        "type_of_mineral_fertilizer_field_",
	        p
	      )]],
	    
	    OM_type =
	      r4[[paste0(
	        "organic_inputs_applied_field_",
	        p
	      )]],
	    
	    inoculated =
	      r4[[paste0(
	        "rhizobium_inoculant_applied_field_",
	        p
	      )]],
	...
	d4$fertilizer_date <- rep(
	  as.Date(
	    paste(
	      2013,
	      r4$date_of_mineral_fertiliser_application_mm,
	      r4$date_of_mineral_fertiliser_application_dd,
	      sep = "-"
	    ),
	    format = "%Y-%m-%d"
	  ),
	...
	d4$flowering_date <- rep(
	  as.Date(
	    paste(
	      2013,
	      r4$`50pct_flowering_mm`,
	      r4$`50pct_flowering_dd`,
	      sep = "-"
	    ),
	    format = "%Y-%m-%d"
	  ),
	...
	d4$maturity_date <- rep(
	  as.Date(
	    paste(
	      2013,
	      r4$`50pct_maturity_mm`,
	      r4$`50pct_maturity_dd`,
	      sep = "-"
	    ),
	    format = "%Y-%m-%d"
	  ),
	...
	d4$harvest_date <- rep(
	  as.Date(
	    paste(
	      2013,
	      r4$date_of_final_harvest_mm,
	      r4$date_of_final_harvest_dd,
	      sep = "-"
	    ),
	    format = "%Y-%m-%d"
	  ),
	...
	d4$weeding_date <- as.Date(
	  paste(
	    2013,
	    r4$date_of_1st_weeding_mm,
	    r4$date_of_1st_weeding_dd,
	    sep = "-"
	  ),
	  format = "%Y-%m-%d"
	)
