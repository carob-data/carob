...
## ISSUES
# fertilizer use is a treatment, but there is no amount and type
# no location data
# no planting date 
...
"
	...
	d$land_prep_method <- gsub("Single plough Flat", "single ploughing", d$land_prep_method)
	d$land_prep_method<- gsub("Single plough Ridged", "single ploughing;ridge tillage", d$land_prep_method)
	d$land_prep_method <- gsub("Double plough Flat", "double ploughing", d$land_prep_method)
	d$land_prep_method<- gsub("Double plough Ridged", "double ploughing;ridge tillage", d$land_prep_method)
	d$land_prep_method <- gsub("Zero tillage Flat", "none", d$land_prep_method)
	d$land_prep_method <- gsub("Zero tillage Ridged", "ridge tillage", d$land_prep_method)
		
	##### assumed herbicide timing = wether following the protocol or farmers choice
## probably correct, but not useful without knowing what these really are
