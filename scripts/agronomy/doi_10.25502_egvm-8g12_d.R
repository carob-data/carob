...
"
	...
	pub <- "https://cgspace.cgiar.org/server/api/core/bitstreams/12584360-c080-4d4f-a356-255bc2c8fc8e/content"
	...
	d <- data.frame(
		season = r$season,
		rep = r$rep,
		adm2 = carobiner::fix_name(r$site, "title"),
		location = carobiner::fix_name(r$epa, "title"),
		...
		trial_id = paste(r$location, r$ID, sep = "-"),
		irrigated = NA,
		yield_moisture = 12.5,
...
## from publication
	...
	geo <- data.frame(
	   location = c("Chinguluwe", "Chipoka", "Thumbwe", "Mombezi", "Dzaone", "Thondwe", "Lisasadzi", "Mtunthama", "Thuchila", "Boma", "Chilipa", "Namkumba", "Zidyana", "Linga"),
