...
"
  ...
  meta <- carobiner::get_metadata(uri, path, group, major=NA, minor=NA,
                                  ...
                                  response_vars = "dmy_storage;dmy_total;harvest_index",
                                  notes = NA,
  ...
  r1 <- read.csv(f1)
  r3 <- read.csv(f3)
  ## r2/r4 are reference/dictionary files, not data - not used further.
  
  ### Fertilizer/irrigation/variety, derived into d1 (r1 itself untouched)
  d1 <- data.frame(
  ...
  d <- data.frame(
    ...
    irrigated = d1$irrigated,
    N_fertilizer = d1$N_fertilizer,
    P_fertilizer = d1$P_fertilizer,
    K_fertilizer = d1$K_fertilizer,
    fertilizer_used = d1$fertilizer_used,
    ...
    harvest_index = r1$HI_OK_roots * 100,
    ...
    Planting_stake_DM = r1$Planting_stake_DM * 1000,
    HI_total = r1$HI_total * 100,
  ...
  d3$trial_id <- 1
  d3$trial_id <- as.character(d3$trial_id)
  #d3$plot_id <- NULL
  
  carobiner::write_files(path, meta, d, d3)
}