...
### process soil data 
   ...
   d$land_prep_method <- ifelse(grepl("no-till", d$land_prep_method), "minimum tillage", "disk tillage")
	...
	### record weather data 
	   ...
	   i <- grep("d", r3$AGRO)
	   r_list <- list(r3[1:(i[2]-2),],  r3[(i[2]-2):(i[3]-2),], r3[(i[3]-2):(i[4]-2),],  r3[(i[4]-2):nrow(r3),])
  ...
  proc_wth <- function(r){
     ...
     if (length(ii) > 1) {
        ...
        names(r) <- new_names
        names(r) <- ifelse(is.na(names(r)), "X", names(r))  
        r <- r[-c(1:3),]
     }
     else {
  ...
  rr <- lapply(r_list, proc_wth)
  rr <- do.call(carobiner::bindr, rr)
  ...
  pr_wth <- function(i, d){
     ...
     wth <- d[grepl(i, d$year),]
     if (i=="2012"){ group <- rep(1:366, length.out = nrow(wth))}
  ...
  date <- c("2010","2011", "2012","2013","2014")
	...
	wth1$year <- as.numeric(gsub("ate/t|ORD\\(N|HPLAT|AGROF|sed 0|a2560", NA, wth1$year))
	wth1$day <- as.numeric(gsub("d|ORT|ONC|ORT|EAD|e U", NA, wth1$day))
	## drop rows with NA in year 
	wth1 <- wth1[!is.na(wth1$year),]
	### fixing date
	wth_mean1 <- lapply(date, function(y) pr_wth(y, wth1))
	...
	wth2 <- wth2[-c(1:2),]	
	wth2$year <- as.numeric(gsub("ate/t|ORD\\(N|HPLAT|AGROF|sed 0|a2560", NA, wth2$year))
	wth2$day <- as.numeric(gsub("d|ORT|ONC|ORT|EAD|e U", NA, wth2$day))
	## drop rows with NA in year 
	wth2 <- wth2[!is.na(wth2$year),]
	
	## fixing date
	wth_mean2 <- lapply(date, function(y) pr_wth(y, wth2))
	...
	wth3$year <- as.numeric(gsub("ate/t|ORD\\(N|HPLAT|AGROF|sed 0|a2560", NA, wth3$year))
	wth3$day <- as.numeric(gsub("d|ORT|ONC|ORT|EAD|e U", NA, wth3$day))
	## drop rows with NA in year 
	wth3 <- wth3[!is.na(wth3$year),]
	
	## fixing date
	wth_mean3 <- lapply(date, function(y) pr_wth(y, wth3))
