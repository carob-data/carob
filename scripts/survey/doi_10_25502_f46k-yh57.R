...
carob_script <- function(path) {
  ...
  build_date <- function(dd, mm, yyyy) {
    ...
    mm_num <- suppressWarnings(as.numeric(mm))
    mm_num[is.na(mm_num)] <- match(tolower(mm[is.na(mm_num)]), tolower(month.name))
    dd_num   <- suppressWarnings(as.numeric(dd))
    yyyy_num <- suppressWarnings(as.numeric(yyyy))
  ...
  lat_field <- suppressWarnings(as.numeric(r1$gps_latitude_field))
  lon_field <- suppressWarnings(as.numeric(r1$gps_longitude_field))
  elv_field <- suppressWarnings(as.numeric(r1[["gps_altitude_field$m"]]))
  lat_hh    <- suppressWarnings(as.numeric(r1[["gps_latitude_hh$decimal_degrees"]]))
  lon_hh    <- suppressWarnings(as.numeric(r1[["gps_longitude_hh$decimal_degrees"]]))
  elv_hh    <- suppressWarnings(as.numeric(r1[["gps_altitude_hh$m"]]))
  ...
  d2$sex     <- sex
  d2$age     <- suppressWarnings(as.numeric(r1[["age_of_farmer$years"]]))
  d2$is_head <- trimws(r1$farmer_hh_head) == "Y"

  d2$hh_adult_women <- suppressWarnings(as.numeric(r1$no_females_17_35_yrs)) +
                        suppressWarnings(as.numeric(r1$no_females_36_60_yrs))
  d2$hh_adult_men   <- suppressWarnings(as.numeric(r1$no_males_17_35_yrs)) +
                        suppressWarnings(as.numeric(r1$no_males_36_60_yrs))
  d2$hh_child_18    <- suppressWarnings(as.numeric(r1$no_females_0_16_yrs)) +
                        suppressWarnings(as.numeric(r1$no_males_0_16_yrs))
  d2$hh_elders      <- suppressWarnings(as.numeric(r1$no_females_over_60_yrs)) +
                        suppressWarnings(as.numeric(r1$no_males_over_60_yrs))
  ...
  edu_mat <- sapply(edu_cols, function(cn) suppressWarnings(as.numeric(r1[[cn]])))
  ...
  d4_list <- lapply(crop_cols, function(cc) {
    yield_kg  <- suppressWarnings(as.numeric(r2[[cc$yield]]))
    area_raw  <- suppressWarnings(as.numeric(r2[[cc$area]]))
  ...
  d5_list <- lapply(plot_index, function(p) {
    ...
    plot_width   <- suppressWarnings(as.numeric(r3[[col_width]]))
    plot_depth   <- suppressWarnings(as.numeric(r3[[col_depth]]))
    grain_kg     <- suppressWarnings(as.numeric(r3[[col_grain]]))
    fwy_total    <- suppressWarnings(as.numeric(r3[[col_bio]]))
    ...
    data.frame(
      ...
      #number_of_rows_in_plot = suppressWarnings(as.numeric(r3[[col_rows]])),
      yield         = yield_kg_ha,
      fwy_total     = fwy_total,
      ...
      fertilizer_amount = rowSums(cbind(
        suppressWarnings(as.numeric(r3[[paste0("fert_1_kg_plot_plot_", p, "$kg_per_plot")]])),
        suppressWarnings(as.numeric(r3[[paste0("fert_2_kg_plot_plot_", p, "$kg_per_plot")]])),
        suppressWarnings(as.numeric(r3[[paste0("fert_3_kg_plot_plot_", p, "$kg_per_plot")]]))
      ), na.rm = TRUE),
      
      inoculant_y_n = yn(r3[[paste0("inoculant_y_n_plot_", p)]]),
      OM_amount     = suppressWarnings(as.numeric(r3[[paste0("manure_kg_plot_plot_", p, "$kg_per_plot")]])),
      crop_1_y_n    = yn(r3[[paste0("crop_1_y_n_plot_", p)]]),
      plant_density = suppressWarnings(as.numeric(r3[[paste0("density_1_plot_", p)]])),
      chemical_y_n  = yn(r3[[paste0("chemical_y_n_plot_", p)]]),
      staking_1     = chr(r3[[paste0("staking_1_plot_", p)]]),
      staking_2     = chr(r3[[paste0("staking_2_plot_", p)]]),
  ...
  dd_field_list <- lapply(seq_along(field_index$field_number), function(i) {
    ...
    data.frame(
      field_id      = r4$farm_id,
      field_number  = fn,
      area_field    = suppressWarnings(as.numeric(r4[[field_cols$area_field[i]]])),
      walking_distance_minutes = suppressWarnings(as.numeric(r4[[field_cols$walking_distance_minutes[i]]])),
      other_crops   = standardise_crop(r4[[field_cols$other_crops[i]]]),
      amount_mineral_fertilizer_applied = suppressWarnings(as.numeric(r4[[field_cols$amount_mineral_fertilizer_applied[i]]])),
  ...
  dd_cropfield_list <- lapply(seq_len(nrow(crop_field_index)), function(i) {
    ...
    data.frame(
      ...
      area_perc    = suppressWarnings(as.numeric(r4[[area_col]])),
  ...
  d$treatment_description <- d$chemical_y_n <- d$inoculant_y_n <- d$staking_1 <- d$staking_2 <- NULL
