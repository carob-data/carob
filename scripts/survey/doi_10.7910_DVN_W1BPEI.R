...
"
BSF finger millet crowdsourcing data for Nyando Kenya
...
"
	...
	uri <- "doi:10.7910/DVN/W1BPEI"
	group <- "survey"
	ff  <- carobiner::get_data(uri, path, group)

	meta <- carobiner::get_metadata(uri, path, group, major=1, minor=1,
		data_organization = "BIOV; KALRO; NCSB", 
		publication = NA,
		project = NA,
	...
	f <- ff[basename(ff) == "Crowdsourcing data for finger millet in Nyando Kenya, 2019.xlsx"]
  ...
  ### Process
	...
	d1 <- data.frame(
		...
		maturity_days = r1$`Days to maturity`,
